...
const ProjectRoadmap = () => {
  ...
  const filteredPhases = phases.map((phase) => {
    const filteredProjects = phase.projects.filter((project) => {
      const matchesSearch =
    ...
    return { ...phase, projects: filteredProjects };
  });
    ...
    <div className="min-h-screen p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          {/* Filters */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 gap-4 sm:gap-0 w-full sm:w-auto">
            {/* Group By (kept for now, not wired to logic) */}
            <div className="relative w-full sm:w-auto">
              <select
                className="appearance-none bg-[#EDEDEF] border border-gray-300 rounded px-4 py-2 pr-8 w-full sm:w-auto focus:outline-none focus:ring-2 focus:ring-green-500"
                value={groupBy}
        ...
        {filteredPhases.map(
          (phase) =>
            phase.projects.length > 0 && ( // ✅ only show non-empty phases
              <div key={phase.id} className="mb-8">
                {/* Phase Header */}
                <div className="flex items-center justify-between mb-4">
                  <div className="bg-[#1D6953] text-white px-4 py-2 rounded-lg font-medium">
        ...
        {selectedProject && (
          <ProjectDetailModal
            isOpen={isModalOpen}
            onClose={closeModal}
            project={selectedProject} // ✅ pass full data
          />
        )}
      </div>
    </div>
